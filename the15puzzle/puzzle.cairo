...
func build_dict(
    ...
    # It's previous location should be where the empty tile is goint to be
    let next_loc: Location* = loc_list + Location.SIZE
    assert dict.prev_value = 4 * next_loc.row + next_loc.col

    # It's next location should be where the empty tile is now
...
func check_solution{output_ptr: felt*, range_check_ptr}(
    ...
    # Start by veryfying that loc_list is valid
    ...
    # Store range_check_ptr in a local variable to make it accessible
    # after the call to output_initial_values()
    local range_check_ptr = range_check_ptr

    # Verify that the squashed dict hast exactly 15 entries
    # This will guarantee that all values in the tile list
